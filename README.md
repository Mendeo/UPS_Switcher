...
Прибор должен получать информацию о наличии напряжения в розетке. Для этого собирается индикатор напряжения в сети в виде обычной неоновой лампочки NE-2 и фоторезистора. Лампочка питается от сети через резистор 470 кОм. Лампочка и фоторезистор помещаются в пластиковый корпус, изготовленный на 3D принтере. Их выводы помещаются в отдельные корпуса, которые служат для изоляции выводов. Ещё две детали служат для помещения в них подводящих проводов: 230В - к лампе и от прибора к фоторезистору. После сборки отдельные части склеиваются цианокрилатным клеем с содой. Я печатал PETG пластиком. Модели отдельных частей, а также исходный код для среды OpenScad лежат в папке [lineDetector](lineDetector)
